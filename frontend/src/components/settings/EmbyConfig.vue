...
onMounted(async () => {
  try {
    ...
    const d = res.data || {}
    config.emby_host = d.emby_host || ''
    config.emby_api_key = d.emby_api_key || ''
    config.emby_user_id = d.emby_user_id || ''
    config.max_actors_per_media = typeof d.max_actors_per_media === 'number' ? d.max_actors_per_media : 50
    config.enable_emby_avatar_first = !!d.enable_emby_avatar_first
    // 归一化 Provider 列表：兼容旧配置数据，backfill alt_base_url 与 UI 临时 _dragId
    const providers = Array.isArray(d.ai_providers) ? d.ai_providers : []
    config.ai_providers = providers.map((p: AIProvider) => ({
      ...p,
...
// 保存前剥离 UI 临时字段 _dragId + 只提交本 Tab 负责的配置键（后端 /api/config 浅合并）
const cleanPayload = () => ({
  emby_host: config.emby_host,
  emby_api_key: config.emby_api_key,
  emby_user_id: config.emby_user_id,
  max_actors_per_media: config.max_actors_per_media,
  enable_emby_avatar_first: config.enable_emby_avatar_first,
  ai_providers: (config.ai_providers || []).map(({ _dragId, ...p }) => p)
})
...
const testConnection = async () => {
  ...
  try {
    await axios.post(`${API_URL}/api/libraries`, {
      emby_host: config.emby_host,
      emby_api_key: config.emby_api_key,
      emby_user_id: config.emby_user_id
    })
...
<template>
  <div class="settings-root">
    ...
    <!-- ==================== 吸顶操作栏（保留本 Tab 独立保存/测试逻辑） ==================== -->
    <div class="sticky-bar">
      <div class="min-w-0">
        <h1 class="page-title">Emby & AI</h1>
        <p class="mt-0.5 text-xs tracking-widest text-slate-500">媒体服务器连接 · 智能模型配置</p>
      </div>
