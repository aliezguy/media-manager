...
import {
  ...
  ArrowUp, ArrowDown, RefreshCw, Globe, UserRound, Lock, KeyRound, Save, ClipboardList, Database
...
// 洗版/订阅策略（与后端 config.yaml wash_schemes / subscribe_schemes 结构一致）
interface MpScheme {
  _dragId?: number
  name: string
  keywords: string[]
    ...
    <section class="glass-card">
      ...
      <div class="card-body">
        <div class="grid grid-cols-1 gap-x-6 gap-y-5 md:grid-cols-2">
          <div>
            <label class="field-label"><Globe :size="13" />TMDB API 地址</label>
            <el-input v-model="config.tmdb_base_url" placeholder="https://api.tmdb.org/3" />
            <p class="field-tip">TMDB v3 API 基础地址；切换代理网关时改这里</p>
          </div>
          <div>
            <label class="field-label"><Globe :size="13" />豆瓣 API 总开关</label>
          ...
          <div class="md:col-span-2">
            ...
            <el-input
              ...
              type="textarea"
              :rows="3"
              placeholder="浏览器登录豆瓣后，从开发者工具复制 Cookie 串"
            />
            <p class="field-tip">用于绕过豆瓣 need_login 流控限制；含敏感信息，请勿外泄</p>
          </div>
        </div>
      </div>
    </section>
